...
function Checkout() {
  ...
  const getAddressByLatLng = async (lat, lng) => {
    try {
      ...
      const result = await axios.get(`https://api.geoapify.com/v1/geocode/reverse?lat=${lat}&lon=${lng}&format=json&apiKey=${import.meta.env.VITE_GEOLOCATION_API_KEY}`)
      dispatch(setAddress(result.data.results[0].address_line1 + result.data.results[0].address_line2));
      setAddressInput(result.data.results[0].address_line1 + result.data.results[0].address_line2)
    } catch (error) {
      console.log(error)
    }
  }

  const getCurrentLatLang = async () => {
  ...
  const getLatLonByAddress = async () => {
    try {
      const result = await axios.get(`https://api.geoapify.com/v1/geocode/search?text=${addressInput}&format=json&apiKey=${import.meta.env.VITE_GEOLOCATION_API_KEY}`);
      ...
      <div className='w-full max-w-225 bg-white rounded-2xl shadow-xl p-6 space-y-6'>
        ...
        <section>
          ...
          <div className='flex gap-2 mb-3'>
            ...
            <button className='bg-[#ff4d2d] hover:bg-[#e64526] text-white px-3 py-2 rounded-lg flex items-center justify-center' onClick={() => getLatLonByAddress()} ><IoSearchOutline size={17} /></button>
            <button className='bg-blue-500 hover:bg-blue-600 text-white px-3 py-2 rounded-lg flex items-center justify-center' onClick={() => getCurrentLatLang()} ><TbCurrentLocation size={17} /></button>
          </div>
