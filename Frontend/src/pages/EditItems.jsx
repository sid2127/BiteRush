...
function EditItems() {
    ...
    const itemDetails = useSelector(state => state.shop.items.find(item => item._id === itemId));

    const [name, setName] = useState(itemDetails?.name || "");
    const [category, setCategory] = useState(itemDetails?.category || "");
    const [price, setPrice] = useState(itemDetails?.price || 0);
    const [foodType, setFoodType] = useState(itemDetails?.foodType || "");
    const [frontendImage, setFrontendImage] = useState(itemDetails?.imageUrl || "");
